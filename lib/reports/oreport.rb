...
#TODO exclude repeated code
		...
		def self.torg12(orderinfo)
			...
			report = ODFReport::Report.new("app/assets/reports/torg-12.odt") do |r|
				...
				#Organization from fields
				r.add_field :to, @ohash["to"].short_name
				r.add_field :to_inn, @ohash["to"].inn
				r.add_field :to_kpp, @ohash["to"].kpp
				r.add_field :to_law_address, @ohash["taddress"]
				...
				#Bank acc fields from 
				r.add_field :bank_to_ks, @ohash["bankacc_to"][0].ks
				r.add_field :bank_to_rs, @ohash["bankacc_to"][0].rs
				...
				r.add_table("TABLE_1", @ohash["orderlines"], :header=>true) do |t|
					...
					t.add_column (:item_qty) {|order_line| nc.number_with_precision(order_line.qty, :precision => 5, :significant => true, :strip_insignificant_zeros  => true)}
					t.add_column (:item_qty_human) {|order_line| RuPropisju.propisju_shtuk(nc.number_with_precision(order_line.qty, :precision => 5, :significant => true, :strip_insignificant_zeros  => true), 1, [])}
					t.add_column (:price) {|order_line| nc.number_to_currency(order_line.price*(1.0-1.0/118.0*18.0),:unit => "")}
					t.add_column (:price_vat) {|order_line| nc.number_to_currency(order_line.price*(1.0-1.0/118.0*18.0)*order_line.qty,:unit => "")}
					t.add_column (:vat) {|order_line| nc.number_to_currency(order_line.price/118.0*18.0*order_line.qty,:unit => "")}
					t.add_column(:sumprice) {|order_line| nc.number_to_currency(order_line.sum_price,:unit => "")}
					t.add_column(:sku) {|order_line| order_line.product.sku[:name]}
				end
				...
				#Totals and number to words lines
				r.add_field :total, nc.number_to_currency(@ohash["order"].total_price, :unit => "")
				r.add_field :total_vat, nc.number_to_currency((@ohash["order"].total_price)*(1.0-1.0/118.0*18.0), :unit => "")
				...
				#Signatures 
				r.add_field :sign1, @ohash["sign_1"].short_name
				r.add_field :post1, @ohash["sign_1"].post
				r.add_field :sign2, @ohash["sign_2"].short_name
			...
			@report=Hash.new
			@report["report"] = report.generate
			@report["file_name"] = "Накладная_№" + @ohash["order"].number + "_" + @ohash["from"].name + "-" + @ohash["to"].name + "_" + @ohash["order"].document_date.strftime("%d.%m.%Y") + "_на_" + nc.number_to_currency(@ohash["order"].total_price, :unit => "") + ".odt"
